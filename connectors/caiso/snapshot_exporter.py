"""
CAISO snapshot exporter for program MVP metrics.
...
class CAISOSnapshotExporter:
    """Export CAISO data to program MVP CSV contracts."""

    def __init__(self, config: CAISOConnectorConfig):
        self.config = config
        self.extractor = CAISOExtractor(config)
        
        # UCAP factors for WECC/CAISO
        self.ucap_factors = {
            "solar_pv": 0.5,
            "wind": 0.3,
            "hydro": 0.9,
    ...
    async def export_snapshot(
        ...
        """Export CAISO snapshot to CSV contracts."""
        ...
        logger.info("CAISO snapshot export completed", output_dir=str(output_dir))

    async def _extract_load_data(
        self, start_time: datetime, end_time: datetime
    ) -> List[Dict[str, Any]]:
        """Extract hourly load data from OASIS."""
        try:
            ...
            # Aggregate load by zone (sum of LMP zones)
            load_by_zone = {}
            for record in result.records:
                timestamp = record.get("timestamp")
                zone = record.get("zone", "CAISO")
            ...
            logger.error("Failed to extract CAISO load data", error=str(e))
            return []

    async def _extract_generation_data(
        self, start_time: datetime, end_time: datetime
    ) -> List[Dict[str, Any]]:
        """Extract hourly generation data from OASIS."""
        try:
            result = await self.extractor.extract_data(
                data_type="lmp",
                start_time=start_time,
            ...
            logger.error("Failed to extract CAISO generation data", error=str(e))
            return []
    ...
    async def _export_generation_capacity_csv(self, output_dir: Path) -> None:
        """Export generation capacity CSV with UCAP factors."""
        file_path = output_dir / "generation_capacity.csv"
        
        # Static capacity data for MVP
        capacity_data = [
            {
    ...
    async def _export_rec_ledger_csv(self, output_dir: Path) -> None:
        ...
        rec_data = [
            ...
            {
                "lse": "SOUTHERN_CALIFORNIA_EDISON",
