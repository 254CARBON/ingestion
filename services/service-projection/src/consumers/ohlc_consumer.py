"""
Kafka consumer for OHLC bars in projection service.

This module provides Kafka consumer functionality for consuming OHLC bar data
...
class OHLCConsumerConfig(BaseModel):
    """Configuration for OHLC consumer."""

    bootstrap_servers: str = Field("localhost:9092", description="Kafka bootstrap servers")
    topic: str = Field("aggregation.ohlc.bars.v1", description="Input topic")
    group_id: str = Field("projection-ohlc-consumer", description="Consumer group ID")
...
class KafkaOHLCConsumer:
    """
    Kafka consumer for OHLC bars.

    This consumer processes OHLC bar data from Kafka and projects it
    to the ClickHouse serving layer.
    """

    def __init__(self, config: OHLCConsumerConfig, projection_service: ProjectionService):
        """
    ...
    async def start(self) -> None:
        ...
        try:
            self.logger.info("Starting OHLC Kafka consumer",
            ...
            self.logger.info("OHLC Kafka consumer started successfully")

        except Exception as e:
            self.logger.error("Failed to start OHLC Kafka consumer", error=str(e))
            raise
    ...
    async def stop(self) -> None:
        ...
        try:
            if self.consumer:
                ...
                self.logger.info("OHLC Kafka consumer stopped")
        except Exception as e:
            self.logger.error("Failed to stop OHLC Kafka consumer", error=str(e))
    ...
    async def consume_messages(self) -> None:
        ...
        try:
            self.logger.info("Starting OHLC message consumption")

            async for message in self.consumer:
                await self._process_message(message)

        except Exception as e:
            self.logger.error("Error consuming OHLC messages", error=str(e))
    ...
    async def _process_message(self, message) -> None:
        ...
        try:
            ...
            if not message_data:
                self.logger.warning("Received empty OHLC message")
                return

            # Extract OHLC bars from message payload
            payload = message_data.get("payload", message_data)

            # Handle both single bar and multiple bars
            ...
            ohlc_bars = [bar for bar in ohlc_bars if bar]

            if not ohlc_bars:
                self.logger.debug("No valid OHLC bars in message")
                return

            self.logger.info("Processing OHLC bars",
                           count=len(ohlc_bars),
                           topic=message.topic,
            ...
            # Project OHLC bars
            projection_result = await self.projection_service.project_ohlc_bars(ohlc_bars)
            ...
            self.logger.info("OHLC bars processed successfully",
                           records=projection_result.records_projected,
                           latency_ms=latency_ms)

        except Exception as e:
            self.stats["failed_projections"] += 1
            self.logger.error("Failed to process OHLC message",
                            error=str(e),
                            topic=message.topic,
                            partition=message.partition,
                            offset=message.offset)
