"""
Kafka consumer for rolling metrics in projection service.

This module provides Kafka consumer functionality for consuming rolling metrics data
...
class MetricsConsumerConfig(BaseModel):
    """Configuration for metrics consumer."""

    bootstrap_servers: str = Field("localhost:9092", description="Kafka bootstrap servers")
    topic: str = Field("aggregation.rolling.metrics.v1", description="Input topic")
    group_id: str = Field("projection-metrics-consumer", description="Consumer group ID")
...
class KafkaMetricsConsumer:
    """
    Kafka consumer for rolling metrics.

    This consumer processes rolling metrics data from Kafka and projects it
    to the ClickHouse serving layer.
    """

    def __init__(self, config: MetricsConsumerConfig, projection_service: ProjectionService):
        """
    ...
    async def start(self) -> None:
        ...
        try:
            self.logger.info("Starting metrics Kafka consumer",
            ...
            self.logger.info("Metrics Kafka consumer started successfully")

        except Exception as e:
            self.logger.error("Failed to start metrics Kafka consumer", error=str(e))
            raise
    ...
    async def stop(self) -> None:
        ...
        try:
            if self.consumer:
                ...
                self.logger.info("Metrics Kafka consumer stopped")
        except Exception as e:
            self.logger.error("Failed to stop metrics Kafka consumer", error=str(e))
    ...
    async def consume_messages(self) -> None:
        ...
        try:
            self.logger.info("Starting metrics message consumption")

            async for message in self.consumer:
                await self._process_message(message)

        except Exception as e:
            self.logger.error("Error consuming metrics messages", error=str(e))
    ...
    async def _process_message(self, message) -> None:
        ...
        try:
            ...
            if not message_data:
                self.logger.warning("Received empty metrics message")
                return

            # Extract rolling metrics from message payload
            payload = message_data.get("payload", message_data)

            # Handle both single metric and multiple metrics
            if isinstance(payload, list):
                rolling_metrics = payload
            else:
                rolling_metrics = [payload]

            # Filter out empty metrics
            rolling_metrics = [metric for metric in rolling_metrics if metric]

            if not rolling_metrics:
                self.logger.debug("No valid rolling metrics in message")
                return

            self.logger.info("Processing rolling metrics",
                           count=len(rolling_metrics),
                           topic=message.topic,
                           partition=message.partition,
                           offset=message.offset)

            # Project rolling metrics
            projection_result = await self.projection_service.project_rolling_metrics(rolling_metrics)
            ...
            self.logger.info("Rolling metrics processed successfully",
                           records=projection_result.records_projected,
                           latency_ms=latency_ms)

        except Exception as e:
            self.stats["failed_projections"] += 1
            self.logger.error("Failed to process metrics message",
                            error=str(e),
                            topic=message.topic,
                            partition=message.partition,
                            offset=message.offset)
