"""
Kafka producer for the normalization service.

This module handles publishing normalized market data to Kafka topics
...
class KafkaProducerService:
    """
    Kafka producer service for the normalization service.
    
    This service handles publishing normalized market data to Kafka topics
    with proper schema validation and serialization.
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.producer_config = {
            ...
            'client.id': f"normalization-producer-{uuid4().hex[:8]}",
            'acks': 'all',
            'retries': 3,
    ...
    async def start(self) -> None:
        ...
        try:
            self.logger.info("Starting normalization Kafka producer service", 
            ...
            self.logger.info("Normalization Kafka producer service started successfully")
            
        except Exception as e:
            self.logger.error("Failed to start normalization Kafka producer service", error=str(e))
            raise
    ...
    async def stop(self) -> None:
        ...
        try:
            self.logger.info("Stopping normalization Kafka producer service")
            ...
            self.logger.info("Normalization Kafka producer service stopped")
            
        except Exception as e:
            self.logger.error("Error stopping normalization Kafka producer service", error=str(e))
    ...
    def _setup_avro_serializer(self, schema_str: str) -> None:
        ...
        try:
            if not self.schema_registry_client:
                raise Exception("Schema Registry client not initialized")
    ...
    async def publish_normalized_record(self, record: Dict[str, Any], schema_str: Optional[str] = None) -> bool:
        """
        Publish a normalized record to Kafka.
        
        Args:
            record: Normalized record data to publish
        ...
        try:
            ...
            self.logger.debug("Normalized record published successfully", 
                            event_id=envelope["event_id"],
                            topic=self.output_topic)
            return True
            
        except Exception as e:
            self.logger.error("Failed to publish normalized record", error=str(e), record=record)
            return False
    ...
    async def publish_batch(self, records: List[Dict[str, Any]], schema_str: Optional[str] = None) -> int:
        """
        Publish a batch of normalized records to Kafka.
        
        Args:
            records: List of normalized records to publish
            schema_str: Avro schema string (optional)
        ...
        try:
            self.logger.info("Publishing batch of normalized records", 
                           count=len(records), 
                           topic=self.output_topic)
            
            for record in records:
                if await self.publish_normalized_record(record, schema_str):
                    success_count += 1
