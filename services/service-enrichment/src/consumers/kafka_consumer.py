"""
Kafka consumer service for enrichment service.

This module provides Kafka consumer functionality for consuming normalized
market data and processing it through the enrichment pipeline.
"""
...
class ConsumerConfig(BaseModel):
    ...
    group_id: str = Field("enrichment-service", description="Consumer group ID")
...
class KafkaConsumerService:
    """
    Kafka consumer service for enrichment.
    
    This service consumes normalized market data from Kafka topics and
    processes it through the enrichment pipeline.
    """
    
    def __init__(self, config: ConsumerConfig, enrichment_service: EnrichmentService):
        """
    ...
    async def _process_message(self, message) -> None:
        ...
        try:
            ...
            # Enrich the data
            enrichment_result = await self.enrichment_service.enrich(message_data)
            
            # Call processing callback if set
            if self.processing_callback:
                await self.processing_callback(enrichment_result)
    ...
    def set_processing_callback(self, callback: Callable) -> None:
        """
        Set processing callback for enriched data.
        
        Args:
            callback: Callback function to call with enriched data
        """
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            **self.stats,
            ...
            "success_rate": (
                self.stats["successful_enrichments"] / max(1, self.stats["total_messages"])
            ),
