"""
Reprocess API endpoint for enrichment service.
...
class ReprocessRequest(BaseModel):
    ...
    data_type: Optional[str] = Field(None, description="Data type filter (optional)")
        ...
        while processed_records < max_records:
            ...
            for i in range(batch_records):
                ...
                mock_data = {
                    ...
                    "data_type": request.data_type or "trade",
                    "price": 45.50 + (i % 100),
                    "quantity": 100 + (i % 1000),
                ...
                # Enrich the data
                enrichment_result = await enrichment_service.enrich(mock_data)
                
                # In production, this would publish to Kafka
                # await kafka_producer.publish_enriched_data(enrichment_result)
